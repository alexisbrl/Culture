...
- **Une question ne compte pour un couple que si la grappe entière est à portée.** Une question qui vise « notion 1, niveau 2 » ne compte pas pour ce couple si elle mobilise par ailleurs une notion hors d'atteinte du membre : elle se pose d'un bloc, donc elle est indisponible en entier. C'est ce point qui interdit de compter question par question.
...
- **Déclenchement : un seul couple à 1 ou 0 question disponible suffit**, pour le membre le plus démuni. **La recharge remet alors TOUS les couples du chapitre à 4** — un déclenchement remet tout au propre, sinon on rechargerait un exercice sur deux.
- **Le radar tourne au lancement d'un exercice**, sur le chapitre concerné, et pour tous les membres de l'atelier — pas seulement celui qui lance.
