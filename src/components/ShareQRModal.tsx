...
export default function ShareQRModal({ open, onClose, title, url }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: ink(0.5), backdropFilter: 'blur(4px)', padding: 16 }} onClick={onClose}>
      <div style={{ background: palette.paper, borderRadius: 20, boxShadow: '0 30px 80px rgba(45,42,36,0.18)', padding: 24, width: '100%', maxWidth: 360, fontFamily: 'inherit', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20 }}>
          <div style={{ padding: 14, background: palette.paper, borderRadius: 14, border: '1px solid rgba(45,42,36,0.10)' }}>
            {url && <QRCodeCanvas ref={canvasRef} value={url} size={180} bgColor="#ffffff" fgColor="#2d2a24" level="M" />}
          </div>
        </div>

        <div style={{ display: 'flex', gap: 8 }}>
          <input readOnly value={url} onFocus={(e) => e.target.select()} style={{ flex: 1, minWidth: 0, fontSize: 12, fontFamily: 'ui-monospace, monospace', padding: '10px 12px', border: '1px solid rgba(45,42,36,0.14)', borderRadius: 10, outline: 'none', background: ink(0.03), color: palette.inkMuted }} />
          ...
          <button onClick={handleDownload} aria-label="télécharger le QR code" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '10px 12px', borderRadius: 10, background: 'transparent', color: palette.inkMuted, border: '1px solid rgba(45,42,36,0.14)', cursor: 'pointer', fontFamily: 'inherit' }}>
            <Download size={14} />
          </button>
        </div>
      </div>
