...
function heatColor(m: number) {
  if (m >= 0.8) return palette.green;
  if (m >= 0.6) return palette.greenSoft;
...
function AKicker({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: palette.inkSoft, marginBottom: 12 }}>
      {children}
    </div>
  );
}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {SECTIONS.map(s => (
              <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 11, color: palette.inkFaint, width: 18 }}>{s.n}</span>
                <span style={{ fontSize: 12.5, color: palette.ink, width: 150 }}>{s.name}</span>
                <div style={{ flex: 1, height: 16, borderRadius: 5, background: ink(0.05), overflow: 'hidden' }}>
                  <div style={{ width: `${s.mastery * 100}%`, height: '100%', background: heatColor(s.mastery), borderRadius: 5 }} />
                </div>
                <span style={{ fontSize: 12, color: palette.ink, fontWeight: 500, width: 38, textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' }}>{Math.round(s.mastery * 100)}%</span>
              </div>
            ))}
          </div>
        </ACard>
            ...
            {DIST.map((d, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, height: '100%', justifyContent: 'flex-end' }}>
                  <span style={{ fontSize: 11, color: palette.ink, fontWeight: 500 }}>{d}</span>
                  <div style={{ width: '100%', height: `${(d / max) * 100}%`, background: i === 2 ? palette.amber : plantAccent, borderRadius: '5px 5px 0 0', minHeight: 4 }} />
                  <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 10, color: palette.inkFaint }}>{SECTIONS[i]?.n}</span>
                </div>
              );
            })}
          </div>
        ...
        {MEMBERS.map((m, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.6fr 1.4fr 2fr 1fr 0.8fr', gap: 12, padding: '11px 18px', alignItems: 'center', borderBottom: i === MEMBERS.length - 1 ? 'none' : `1px solid ${ink(0.05)}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 12.5, color: palette.ink, fontWeight: 500 }}>{m.name}</div>
                <div style={{ fontSize: 10, color: palette.inkFaint }}>{m.role}</div>
              </div>
            </div>
            <span style={{ fontSize: 12, color: palette.inkMuted }}>{m.section}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ flex: 1, height: 6, borderRadius: 3, background: ink(0.06), overflow: 'hidden' }}>
                <div style={{ width: `${m.pct}%`, height: '100%', background: m.pct === 100 ? palette.green : plantAccent }} />
              </div>
              <span style={{ fontSize: 11.5, color: palette.ink, fontVariantNumeric: 'tabular-nums', width: 30, textAlign: 'right' as const }}>{m.pct}%</span>
            </div>
            <span style={{ fontSize: 11.5, color: palette.inkSoft }}>{m.last}</span>
            <span style={{ fontSize: 12.5, color: m.note === '—' ? palette.inkGhost : palette.ink, fontWeight: 500, textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' }}>{m.note}</span>
          </div>
        ))}
      </ACard>
    </div>
