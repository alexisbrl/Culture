...
>
> **Tout le contenu est encore mocké en mémoire** (pas de persistance Supabase) — à brancher lors de l'implémentation réelle du générateur d'examen.
>
...
> - **Filtres avancés** : la barre de recherche « filtrer les questions… » est désormais un `<input>` fonctionnel (`search`, filtre sur `q.content` insensible à la casse). À côté, un bouton « filtres ▾ » (badge `(N)` si des filtres actifs) ouvre un popover avec deux catégories multi-sélectionnables : **Type de question** (chips générées dynamiquement à partir des types présents dans `questions`, via `RESPONSE_TYPE_LABELS`) et **Difficulté** (3 buckets `facile` 1-3 / `moyen` 4-7 / `difficile` 8-10, via la fonction `diffBucket(v)` et `DIFF_BUCKET_LABELS`). Un lien « réinitialiser » vide `filterTypes`/`filterDiffs`. Les trois catégories de filtre (libellés via les chips, type, difficulté) et la recherche texte sont combinées en ET.
> - **Tag `#tombé en examen` → `#jamais tombé en examen`** : renommé dans `INITIAL_POOLS` (id `examQ` conservé). Comme tous les pools, ce libellé est désormais filtrable via les chips de la banque.
