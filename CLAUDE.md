...
> **Nettoyage de la corbeille via pg_cron (audit §4.3)** [AJOUTÉ PAR CLAUDE - 25/06/2026] : la suppression définitive des ateliers en corbeille depuis > 7 jours est une **tâche planifiée côté base** (extension `pg_cron`, migration `schedule_trash_cleanup_cron`), **plus du code applicatif**. Job nommé `cleanup-expired-trashed-workshops`, tous les jours à **03:00 UTC**, qui exécute `delete from public.workshops where deleted_at is not null and deleted_at < now() - interval '7 days'`. **Ne PAS réintroduire** d'appel de nettoyage dans `getUserWorkshops` ni dans un autre chemin de lecture (c'était le problème corrigé : il tournait à chaque chargement de dashboard de chaque utilisateur). Pour modifier la fréquence/la rétention : `select cron.schedule('cleanup-expired-trashed-workshops', '<cron>', '<sql>')` (job nommé = mise à jour idempotente, pas de doublon) ; pour l'inspecter : `select * from cron.job` / `cron.job_run_details`. La rétention 7 jours doit rester cohérente avec ce qu'affiche la corbeille du dashboard.
