...
**Dans une grille, la colonne aussi a besoin de `minWidth: 0`.** Une piste de grille vaut `min-width: auto` par défaut : elle ne descend pas sous la largeur minimale de son contenu, et un texte en `nowrap` l'élargit donc bien au-delà de son `fr`. Le texte n'est alors jamais coupé — c'est la colonne qui cède, et les colonnes suivantes sortent de l'écran. Le `minWidth: 0` du texte ne suffit pas : il faut aussi celui de la piste. Précédent : la page paramètres « Chapitre & Notion », dont un nom de chapitre long poussait la colonne des notions hors de l'écran (19/08/2026).

**L'infobulle de secours ne se déduit alors plus du texte** : la coupe se décide au rendu. La mesure vit dans `useIsClipped` (`src/components/ui/clipped-text.tsx`) — `scrollWidth > clientWidth + 1` (la tolérance d'un pixel écarte les arrondis sous-pixel) dans un `useLayoutEffect` sans dépendances + `ResizeObserver`, gardée par une comparaison explicite (voir plus bas). Sans cette mesure, il ne reste que deux mauvaises options : une infobulle sur tout, y compris ce qui se lit déjà en entier, ou aucune sur ce qui est coupé. Pour une ligne de texte ordinaire, prendre le composant `<ClippedText text=… />` du même fichier, qui pose la coupe et l'infobulle d'un coup ; `LabelPill` (`examen/examShared.tsx`) n'utilise que le hook, sa pastille ayant sa propre structure.

## Un conteneur qui défile rogne AUSSI l'axe horizontal

`overflow-y: auto` ne laisse pas l'autre axe en `visible` : le navigateur le calcule en `auto`, et tout ce qui dépasse latéralement est rogné. Ce qui dépasse d'un élément de pleine largeur, c'est son **halo de focus** (`--shadow-focus`, 3px tout autour) — coupé à gauche et à droite, il ne reste qu'un cadre ouvert. Corriger par une marge intérieure sur le conteneur, rendue au voisinage par une marge négative de même valeur et compensée sur la largeur (`width: 264 = 232 + 2 × 16`) : la colonne occupe exactement la même place et son contenu tombe au même endroit, halo compris. Précédent : la navigation des paramètres (`SettingsClient.tsx`, `.scroll-panel`, 19/08/2026) ; la valeur (16) tient compte du lien de retour, qui déborde de 10px sur la gauche par marge négative.
...
- **Mettre à l'échelle un bloc de largeur fixe selon la place disponible : `zoom`, pas `transform: scale()`.** Précédent : la feuille A4 de l'onglet examen, de 75 % à 140 % par paliers (`--exam-scale`, `.exam-shell` dans `globals.css`). `zoom` agit sur la mise en page — le conteneur occupe exactement la place du contenu mis à l'échelle, sans compensation de hauteur ni marges négatives — et la largeur disponible des enfants est divisée par le facteur, donc un bloc de largeur fixe continue de tomber pile dans son panneau à toutes les échelles (les marges du panneau doivent alors s'exprimer en `calc(… * var(--echelle))` pour rester proportionnelles). Contrairement à `transform`, il ne casse pas les descendants en `position: fixed` (un `inset: 0` couvre toujours la fenêtre entière — vérifié). Côté mesures : **`offsetHeight`/`clientHeight` restent en unités locales (non zoomées), `getBoundingClientRect()` est dans le repère de la fenêtre** — ne jamais mélanger les deux dans un même calcul (une pagination mesurée en `offsetHeight` reste juste à toute échelle ; un centrage de défilement comparé à un `clientHeight` de conteneur non zoomé doit lire la hauteur sur le rectangle). Garder le conteneur défilant **hors** du zoom : ses `scrollTop`/`scrollTo` restent ainsi dans le même repère que les rectangles. Enfin, les paliers se choisissent en CSS pur (media queries sur des variables) plutôt qu'au `ResizeObserver` : pas d'état, pas de saut après hydratation.
  ...
  **Corollaire découvert le 06/09/2026 : `scrollIntoView` ne fonctionne pas sous un ancêtre `zoom`** — l'appel part, le panneau ne bouge pas, et l'élément visé reste hors de l'écran sans la moindre erreur. Viser soi-même le panneau défilant : remonter jusqu'au premier ancêtre `overflow-y: auto|scroll` réellement débordant, calculer l'écart entre les deux rectangles, puis **le ramener en unités locales** en calibrant sur le panneau (`rect.height / clientHeight`) avant de le donner à `scrollTo` — `scrollTop` est en unités locales, un rectangle est dans le repère de la fenêtre. Précédent : le formulaire de question qui s'ouvrait hors de l'écran dans la liste de l'onglet examen (colonne mise à l'échelle par `--exam-list-zoom`), le clic paraissant sans effet.

  **Second corollaire, même famille et même jour : `scrollTo({ behavior: 'smooth' })` peut lui aussi ne rien faire, en silence.** Sur le panneau de la copie A4, l'appel rend la main sans déplacer d'un pixel, alors que la même position posée d'un coup (`panel.scrollTop = top`) fonctionne — vérifié à la console, deux appels d'affilée sur le même élément. **Un recadrage ne doit donc jamais reposer sur la seule animation** : animer, puis repasser ~700 ms plus tard et POSER la position si l'écart est toujours là. C'est la passe de rattrapage qui garantit le résultat, l'animation n'est qu'un confort. Corollaire de méthode : un défilement qu'on croit avoir écrit se vérifie en LISANT `scrollTop` après coup, jamais en regardant le code.
